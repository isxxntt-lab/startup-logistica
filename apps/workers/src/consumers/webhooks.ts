...
export async function handleWebhook(event: WebhookReceived) {
  ...
  await logOps({
    level: "info",
    category: "webhook",
    event: "webhook.worker_start",
    actor: event.proveedor,
    payload: { proveedorMessageId: event.proveedorMessageId ?? null },
  });
  try {
    ...
    if (messageId) {
      ...
      if (rowCount && rowCount > 0) {
        await logOps({
          level: "info",
          category: "webhook",
          event: "webhook.worker_success",
          actor: event.proveedor,
          durationMs: Date.now() - started,
          payload: { statusCode: 200, proveedorMessageId: messageId, matched: true },
        });
        return;
      }
    }
    ...
    await logOps({
      ...
      actor: event.proveedor,
      durationMs: Date.now() - started,
      payload: { statusCode: 200, proveedorMessageId: messageId || null },
    });
  } catch (err) {
    await logOps({
      level: "error",
      ...
      actor: event.proveedor,
      durationMs: Date.now() - started,
      payload: { statusCode: 500, error: (err as Error).message },
    });
    throw err;
  }
}
