...
- [x] HTTPS local del stack prod por override (`docker-compose.prod.local.yml` + `Caddyfile.local` + `tls internal`). El `Caddyfile` de prod **no** lleva `tls internal`. Ver `deploy/LOCAL_HTTPS.md`.
- [x] tmpfs de nginx (`web`) con `uid=101,gid=101` para `read_only` + `nginx-unprivileged`
- [x] Dockerfiles copian fuentes del workspace **antes** de `pnpm install` (evita COPY vs `node_modules` de pnpm)
